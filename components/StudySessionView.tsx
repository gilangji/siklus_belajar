...
import { Loader2, Youtube, Clock, BookOpen, ArrowRight, Save, PlayCircle, Timer, Coffee, Play, PauseCircle, Upload, FileText, X, Settings2, BarChart3, PenTool, MessageSquare, Send, ChevronRight, ChevronLeft, Hourglass, Download, Pause, Sparkles, Music, CloudRain, Flame, Volume2, VolumeX, Gift } from 'lucide-react';
...
interface StudySessionViewProps {
  ...
  // Function to trigger reward
  onUnlockReward: () => { id: string, name: string } | null;
}

const StudySessionView: React.FC<StudySessionViewProps> = ({ initialTopic, onSaveSession, onSessionUpdate, onUnlockReward }) => {
  ...
  // Reward State
  const [newReward, setNewReward] = useState<{ id: string, name: string } | null>(null);
      ...
      <div className="max-w-7xl mx-auto h-[calc(100vh-140px)] flex flex-col animate-fade-in">
        ...
        <div className="flex-1 flex gap-4 overflow-hidden relative">
          <div className={`flex-1 glass-card rounded-xl overflow-hidden flex flex-col relative transition-all duration-300 ${isChatOpen ? 'w-2/3' : 'w-full'}`}>
            ...
            {!isChatOpen && (
              <button onClick={() => setIsChatOpen(true)} className="absolute bottom-8 right-8 bg-primary hover:bg-primaryHover text-white p-4 rounded-full shadow-2xl shadow-primary/40 transition-transform hover:scale-110 flex items-center gap-2 z-10">
